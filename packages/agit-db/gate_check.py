...
# gate_check.py — [전문-바이브코딩] 게이트 판정 (v1.0)

# 판정: G3(비밀정보) · G5(롤백 지점) · G7(비가역 작업)

# 참고: G4(PII)는 --pii 로 '후보'만 출력한다. 스크립트는 G4를 PASS시키지 않는다.

# 사용: python gate_check.py [경로] [--pii]      종료코드 0 = PASS
...
# --- G5 롤백 지점 (ok-* 또는 baseline-* / Greenfield 예외 포함)
...
# --- G7 비가역: 형식 제한 + ID별 up↔down 짝 + 중복 ID 검사

MIG_DIR  = re.compile(r"(migrations?|alembic[/\\]versions|prisma[/\\]migrations)", re.I)

MIG_NAME = re.compile(r"^(\d+)_(?:.+_)?(up|down)\.(sql|py|js|ts)$", re.I)  # 001_create_users_up.sql
...
for p in mig:
    ...
    if not m:                                # Flyway V001__x.sql, Alembic abc123_x.py 등
...
if unparsed:
    ...
    fails.append(f"G7 형식 위반(ID 해석 불가): {sorted(unparsed)} "

                 f"— 규칙: <숫자ID>_<설명>_up|down.<sql|py|js|ts>")
...
if missing:
    ...
    if backup:
        ...
        notes.append(f"G7: backup manifest 발견({backup[0].name}) "

                     f"— **복원 테스트 결과를 사람이 확인해야 PASS**")
